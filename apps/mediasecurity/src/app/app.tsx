...
export function App() {
  ...
  const goToNextStep = () => {
    ...
    (async () => {
      ...
      if (nextStep === Step.QUESTION_QR_CODES) {
        await speak('Коля навел камеру смартфона на QR-код, отсканировал его и перешел по ссылке внутри, после чего открылся сайт с красным окошком внутри.');
        await new Promise((resolve) => setTimeout(resolve, 1000));
        await new Promise((resolve) => setTimeout(resolve, 100));
  ...
  const renderContent = () => {
    ...
    if (step === Step.QUESTION_QR_CODES) {
      return (
        <>
          Коля навел камеру смартфона на QR-код<Typist.Delay ms={500} />, отсканировал его и перешел по ссылке внутри<Typist.Delay ms={1000} />, после чего открылся сайт с красным окошком внутри.<Typist.Delay ms={1000} /><br/>
          <br/>
